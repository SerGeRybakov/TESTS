...
class TestNewUser(Fixtures):
    ...
    def test_chars_in_password(self, chars, new_user):
    ...
    def test_generate_hash(self, password, hash):
        result = generate_hash(password)
        assert hash == result

    def test_create_user(self, new_user, read_mock_db):
        assert new_user._username == read_mock_db[-1]['username']

    def test_create_user_output(self, capsys, new_user):
